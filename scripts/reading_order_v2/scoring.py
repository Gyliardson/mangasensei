...
def _pair_metrics(pairs: list[Pair], observed_position: dict[str, int]) -> PairMetrics:
    wrong = tuple(
        sorted(
            pair
            for pair in pairs
            if observed_position[pair[0]] > observed_position[pair[1]]
        )
    )
    total = len(pairs)
    inversions = len(wrong)
    correct = total - inversions
    accuracy = Fraction(correct, total) if total else Fraction(1, 1)
    normalized = Fraction(inversions, total) if total else Fraction(0, 1)
    return PairMetrics(total, inversions, correct, accuracy, normalized, wrong)


def _all_gt_pairs(sequence: tuple[str, ...]) -> list[Pair]:
    return [
...
def _slice_pairs(pairs: tuple[QualificationPair, ...]) -> dict[str, list[Pair]]:
    result: dict[str, list[Pair]] = {}
    for pair in pairs:
...
def score_page(gt: PageGroundTruth, observed_order: tuple[str, ...]) -> PageScore:
    ...
    missing_all = sorted(expected_all - set(observed_order))
    if unexpected or missing_all:
        raise ValueError(
            f"{gt.page_id}: observed region set mismatch: "
            f"missing={missing_all}, unexpected={unexpected}"
        )
    scored = tuple(region_id for region_id in observed_order if region_id in set(gt.reading_order))
    missing_scored = sorted(set(gt.reading_order) - set(scored))
    if missing_scored:
        raise ValueError(f"{gt.page_id}: missing scored IDs {missing_scored}")
    position = {region_id: index for index, region_id in enumerate(scored)}
    aggregate = _pair_metrics(_all_gt_pairs(gt.reading_order), position)
    slices = {
        name: _pair_metrics(pairs, position)
        for name, pairs in sorted(_slice_pairs(gt.qualification_pairs).items())
    }
    return PageScore(
        page_id=gt.page_id,
        coverage=(
            Fraction(len(scored), len(gt.reading_order))
            if gt.reading_order
            else Fraction(1, 1)
        ),
...
def _combine_metrics(metrics: list[PairMetrics]) -> PairMetrics:
    ...
    return PairMetrics(
        ...
        pairwise_accuracy=Fraction(correct, total) if total else Fraction(1, 1),
        normalized_inversion_distance=Fraction(inversions, total) if total else Fraction(0, 1),
        wrong_pairs=wrong,
    )
...
def candidate_only_wrong_pairs(control: PairMetrics, candidate: PairMetrics) -> tuple[Pair, ...]:
