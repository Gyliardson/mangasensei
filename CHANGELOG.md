...
- Localized JMdict gloss resolver over canonical lexical identities with complete German hits, explicit exact-form English fallback, unsupported-language provenance, and no cross-language sense splicing.
- Durable result-scoped JMdict language projections for `en`, `de`, and explicit `pt-BR` → English fallback, with per-vocabulary provenance and dictionary-only local reprojection that reuses canonical linguistic analysis.
- Independent browser-local `en`/`de`/`pt-BR` dictionary-language preference and reader control, with retained completed results during reprojection, explicit requested/effective language and English-fallback presentation, and real full-stack browser coverage of persisted German reprojection.
...
- The deterministic local JMdict product surface is now English-only: German pack acquisition/runtime selection and the reader dictionary-language control are retired, stale browser preferences normalize to English, unsupported new dictionary requests are rejected, and historical persisted language metadata remains readable for upgrade safety.
...
- OCR regions now follow deterministic local panel/frame flow when reliable, with right-to-left/top-to-bottom panel precedence and the proven manga-tier order as an explicit fallback when panel evidence is weak or ambiguous.
