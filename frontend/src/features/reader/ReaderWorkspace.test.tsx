...
function region(id: string, text: string, order: number): StudyRegion {
  return {
    ...
    confidence: 0.8,
    readingOrder: order,
    tokens: [{ surface: text, lemma: text, reading: order === 0 ? text : "ヨミ", partOfSpeech: "名詞", dictionaryId: null }],
    translation: null,
    explanation: null,
    grammar: [],
    vocabulary: [],
...
describe("ReaderWorkspace", () => {
  ...
  it("uses API reading order for overlay numbers, initial selection and keyboard sequence", async () => {
    const user = userEvent.setup();
    renderWorkspace(page([region("top-right", "上右", 0), region("top-left", "上左", 1), region("lower-right", "下右", 2)]));
    expect(screen.getByRole("heading", { name: "上右" })).toBeVisible();
    const controls = screen.getAllByRole("button", { name: /^Região \d:/ });
    expect(controls.map((control) => control.getAttribute("aria-label"))).toEqual(["Região 1: 上右", "Região 2: 上左", "Região 3: 下右"]);
  ...
  it("suppresses ruby when a kana-only surface differs only by script", () => {
    const kanaRegion: StudyRegion = { ...region("kana", "です", 0), tokens: [{ surface: "です", lemma: "です", reading: "デス", partOfSpeech: "助動詞", dictionaryId: null }] };
  ...
  it("switches furigana script, hides ruby and persists the presentation preference", async () => {
    ...
    const preferenceRegion: StudyRegion = {
      ...
      tokens: [
        { surface: "猫", lemma: "猫", reading: "ネコ", partOfSpeech: "名詞", dictionaryId: null },
        { surface: "です", lemma: "です", reading: "デス", partOfSpeech: "助動詞", dictionaryId: null },
        { surface: "食べる", lemma: "食べる", reading: "タベル", partOfSpeech: "動詞", dictionaryId: null },
      ],
    };
  ...
  it("falls back to hiragana when the stored preference is stale", () => {
    ...
    const localOnlyRegion: StudyRegion = { ...region("stale", "猫", 0), tokens: [{ surface: "猫", lemma: "猫", reading: "ネコ", partOfSpeech: "名詞", dictionaryId: null }] };
  ...
  it("shows local English dictionary meanings independently from contextual study language", () => {
    const localOnlyRegion: StudyRegion = {
      ...
      vocabulary: [{ id: "jmdict-1467640", surface: "猫", lemma: "猫", reading: "ネコ", meanings: ["cat"], source: "JMdict", jlpt: { level: "N5", official: false } }],
    };
    renderWorkspace(page([localOnlyRegion]));
